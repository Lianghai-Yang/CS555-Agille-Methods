...
class Lists(unittest.TestCase):
    ...
    def test_list_recent_deaths(self):
        utils = Utils()
        before_31_days = (datetime.today() - timedelta(days=20)).strftime(_format)
        print(before_31_days)
        self.assertListEqual(utils.list_recent_deaths(
            people = {
                '@I1@': {'ID': '@I1@', 'NAME': 'Tim /James/', 'SEX': 'M', 'BIRT': before_31_days, 'FAMC': '@F2@', 'DEAT': '24 SEP 2019', 'HUSB': ['@F1@'], 'CHIL': ['@F2@']},
